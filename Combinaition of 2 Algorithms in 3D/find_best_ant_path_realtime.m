function [best_ant_path,min_distance] = find_best_ant_path_realtime(all_ant_path,worker_number,task_number,ant_num,Robot_position,Target_position, UAV_speed)
    ...
    for i=1:ant_num
        ...
        if (Lia == 0)
            ...
            for j=1:worker_number
                ...
                SumOfDistance(i)=SumOfDistance(i) + ...
                    sqrt( (Robot_position(j,1)-Target_position(row(allo),1))^2 ...
                    + (Robot_position(j,2)-Target_position(row(allo),2))^2 +...
                    (Robot_position(j,3)-Target_position(row(j),3))^2) / UAV_speed(j);
            end
            %ite= ite+1;
        end
    end
    ...
    for i=1:ant_num
        ...
        if(length(col)-length(unique(col)) == 0)
            for j=1:length(row)
                ...
                SumOfDistance(i)=SumOfDistance(i)+...
                    sqrt( (Robot_position(j,1)-Target_position(row(j),1))^2 ...
