...
for n = 1: size(best_path,1)
    ...
    %title("The intial map view of an example of simulation in 3D");
    ...
    % while (sum(judge~=0))
        for j =1:size(UAV_position,1)
            ...
            + (UAV_position(j,2)-Target_position(best_path(n,j),2))^2 + (UAV_position(j,3)...
            -Target_position(best_path(n,j),3))^2);
            if (dist <= UAV_speed(j))
